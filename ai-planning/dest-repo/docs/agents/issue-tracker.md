...
- **Board Status is derived, never hand-set.** The `canonical/desktop-engineering` board-sync sweep computes each ticket's project **Status** from native GitHub facts on every board event (`closed OR a merged deliberately-linked PR → Done`, `open blocker → Blocked`, **open non-draft linked PR → In review**, `assignee, child in progress, or an open draft linked PR → In progress`, else `Ready`). The PR link the sweep reads is a **deliberate native link** (`closedByPullRequestsReferences(userLinkedOnly: true)`) that the tooling authors automatically once a PR references its ticket (a `Ticket: <url>` field or a closing keyword + `#n`) — never a mention or keyword-inclusive read. **Agents and skills write no Status and add no cards** — assigning the ticket (Claim) is enough to move it to `In progress` even before a PR exists; opening a **draft** PR also moves it to `In progress`, marking it **ready for review** is what moves it to `In review`, and merging moves it to `Done`. Do **not** `gh project item-edit` a Status by hand: the next sweep overwrites it from facts.
