...
- **Link an implementation PR to its planning ticket, cross-repo.** A bare `Closes #n` in a code-repo PR is GitHub-native same-repo closing — it binds to `<owner>/<repo>#n`, never the planning ticket. Reference the ticket in the PR body with a `Ticket: https://github.com/<planning-repo>/issues/<n>` line (preferred) or a fully-qualified `Closes <planning-repo>#n`, so the board-sync link-authoring arm can create the cross-repo link the sweep scores.
