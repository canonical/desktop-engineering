...
- **Board Status is derived, never hand-set.** The `canonical/desktop-engineering` board-sync sweep computes each ticket's project **Status** from native GitHub facts on every board event (`closed → Done`, `open blocker → Blocked`, **open non-draft linked PR → In review**, `assignee or child in progress → In progress`, else `Ready`). **Agents and skills write no Status and add no cards** — assigning the ticket (Claim) is enough to move it to `In progress`; opening a *linked, non-draft* PR is what moves it to `In review`. Do **not** `gh project item-edit` a Status by hand: the next sweep overwrites it from facts.
