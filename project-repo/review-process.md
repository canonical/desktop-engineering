...
1. The pull request should always be opened as a draft, to avoid notifying the reviewers before the tests pass and the pull request is fully ready for review. Ensure the description is concise, readable, and explains the intent behind the changes, including relevant decisions and trade-offs. A description generated only by asking an AI tool to summarise the diff is discouraged: reviewers can inspect or summarise the code themselves, but they need the author's context and reasoning. AI tools may help draft or edit the description, but the submitter is responsible for ensuring that it accurately conveys the intended message. The pull request should be linked to a Jira card, using `UDENG-XXX` in its description, so that Jira links against it.
...
1. The submitter must personally assess and address each review finding. AI tools may help the submitter understand a comment or develop a solution, but review comments must not be delegated directly to an AI agent to resolve. The submitter remains responsible for the response and resulting changes, and should understand those changes and ensure they preserve the author's intent. Reviewers should be discussing the change with its human author, not indirectly writing the final code through an AI agent.
